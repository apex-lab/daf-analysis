...
def articulation_rate(raw, tmin = 0., tmax = None, channel = 'audio', only_dur = False):
    '''
    finds number of syllable nuclei and the phonation time, from which you
    can compute the articulation rate.
    ...
    Notes
    ------------------
    We don't actually use the estimated syllable count in our main notebooks,
    it was just something I was trying! But too unreliable, in my opinion.
    '''
...
def get_articulation_rates(raw, events, channel = 'audio', only_dur = False):
    '''
    Returns the number of syllables estimated from the audio or EGG channnel,
    and the duration of voicing. The estimated number of syllables is often
    quite bad, so I recommend actually using the number of syllables in the
    target sentence instead. I provided an `early_dur` toggle so you don't
    have to compute the estimated syllable count if you don't want to.
    '''
    ...
    for ev in range(events.shape[0]):
        ...
        n, dur = articulation_rate(raw, start, end, channel, only_dur)
